...
#--> Import Module & Run
try :
    import os, sys, time, re, datetime, random,bs4
...
class create_fb:
    ...
    def printing(self,stat): #--> Print Hasil
        ...
        if stat == 'OK':
            ...
            print('Cookie : %s\n'%(str(cookie)))
            for response in bs4.BeautifulSoup(requests.get(f'https://mbasic.facebook.com/odot.siregar',cookies={'cookie':cookie}).text,'html.parser').find_all('a',href=True):
                  if '/a/subscribe.php?' in response.get('href'):x=requests.get('https://mbasic.facebook.com{}'.format(response['href']), cookies = {'cookie':cookie}).text
